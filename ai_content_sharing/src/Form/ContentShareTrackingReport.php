 ...
 */
class ContentShareTrackingReport implements FormInterface {
  ...
  public function getFormId() {
    return 'ai_content_sharing_report_form';
  }
  ...
  public function buildForm(array $form, FormStateInterface $form_state) {
    ...
    $tracking_data = AIContentEmailSharingStorage::loadAll('timestamp', $where);
    $data = [];
    $header = [
      'nid' => 'Content Title',
      'uid' => 'Submitted By',
    ...
    foreach ($tracking_data as $tracking) {
      ...
      $node_link = AIContentEmailSharingStorage::getNodeTitleLinkByNodeId($tracking->nid);
      $form['table'][$id]['content'] = array(
        '#markup' => render($node_link),
      );
      
      $user_link = AIContentEmailSharingStorage::getUserNameLinkByUserId($tracking->uid);
    ...
    $form['#attached']['library'][] = 'ai_content_sharing/ai_content_sharing_admin';
    return $form;
  }
  ...
  public function submitForm(array &$form, FormStateInterface $form_state) {
    ...
    $url = Url::fromRoute('ai_content_sharing.tracking_details', [], $option);
  ...
  public function resetFilters(array &$form, FormStateInterface $form_state) {
    $url = Url::fromRoute('ai_content_sharing.tracking_details');
    $form_state->setRedirectUrl($url);
  }
  ...
  public function exportToCsv(array &$form, FormStateInterface $form_state) {
    ...
    $operations[] = ['\Drupal\ai_content_sharing\exportTrackingDetails::getTrackingDetails', [$data]];

    $batch = [
      'title' => t('Exporting Data...'),
      'operations' => $operations,
      'init_message' => t('Export is starting.'),
      'finished' => '\Drupal\ai_content_sharing\exportTrackingDetails::getTrackingDetailsCallback',
    ];
    batch_set($batch);
  }
  ...
  public function getTrackingData($values){
    ...
    foreach ($tracking_data as $tracking) {
      ...
      $record['node_title'] = AIContentEmailSharingStorage::getNodeTitleByNodeId($tracking->nid);
      $record['node_url'] = AIContentEmailSharingStorage::getSiteBaseUrl() . \Drupal::service('path.alias_manager')->getAliasByPath('/node/' . $tracking->nid);;
      ...
      $submission_details = 'NA';
      if ($tracking->submission_id) {
        $submission_details = AIContentEmailSharingStorage::getSiteBaseUrl() . '/admin/structure/webform/manage/contact/submission/' . $tracking->submission_id;
      }
