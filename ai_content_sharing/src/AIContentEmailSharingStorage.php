 ...
 */
class AIContentEmailSharingStorage {

  /**
   * Save an entry in the ai_content_sharing table.
   *
   * @param array $entry
  ...
  public static function insert(array $entry) {
    $return_value = NULL;
    try {
      $return_value = db_insert('ai_content_sharing')
        ->fields($entry)
        ->execute();
    }
    catch (\Exception $e) {
  ...
  public static function update(array $entry, $key = 'tracking_id') {
    try {
      ...
      $count = db_update('ai_content_sharing')
        ->fields($entry)
  ...
  public static function load($value) {
    $select = db_select('ai_content_sharing', 'tb');
    $select->fields('tb');
  ...
  public function getWebformSubmissionId() {
    ...
    $select->condition('webform_id', 'email_sharing', '=');
