...
namespace Drupal\ai_checklist\AccessChecks;
 ...
 */
class ChecklistAccessCheck implements AccessInterface {
  ...
  public function access($node, AccountInterface $account) {
    ...
    if ($node instanceof Node && $node->bundle() == 'use_case_or_accelerator') {
      $access_list = [];
      ...
      elseif ($account->hasPermission('view any checklist')) {
        if (self::isUserHasChecklistAccess($node)) {
          return AccessResult::Allowed();
        }
      }
    }
   ...
   *
   */
  public static function isUserHasChecklistAccess(Node $node) {
    $published_access = false;
    $current_user = \Drupal::currentUser();
    $current_userid = $current_user->id();
    $roles = $current_user->getRoles();

    if (in_array('administrator', $roles) || in_array('super_webadmin', $roles)) {
      return TRUE;
    }
